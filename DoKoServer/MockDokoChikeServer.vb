...
Private TankOnline As Boolean = True
    ...
    Sub Main()
        ...
        ' ★ 別スレッドでモード切り替えを受け付ける
        Dim modeThread As New Thread(AddressOf HealthModeLoop)
        modeThread.IsBackground = True
        modeThread.Start()
...
End Sub
    ...
    ' =========================
    ' 判定要求 A4/00 に対するレスポンス
    ' （QR判定OKになるように、媒体種別=0x02, 判定結果=0x00 をセット）
    ' =========================
    Private Function BuildJudgmentResponse(reqHeader As Byte(),
        ...
        Const APP_BASE As Integer = 4                      ' アプリ先頭 (Judge側 OFFSET_APP_DATA と合わせる)
        ...
        ' ====== No.1: 処理方向 (1byte) ======
        Dim reqProcDir As Byte = reqData(4)                ' リクエストの処理方向
        ...
        ' 判定日時(6B) → 仮で 2025/12/06 15:15 相当のダミー値
        resultInfo(0) = &H20
        resultInfo(1) = &H25
        resultInfo(2) = &H12
        resultInfo(3) = &H6
        ...
        Dim ymdFrom As Byte() = {&H20, &H25, &H12, &H6, &H0, &H0, &H0, &H0}
        ...
        ' ====== ★ QRチェック用の媒体種別・判定結果をセット ======
        Const REL_MEDIA As Integer = 19      ' JudgeRequestLogic.QRcodeVerification と合わせる
        ...
        For i As Integer = 0 To APP_NO1_TO_NO11_LEN - 1      ' 0 ～ 575
            sum += CUInt(data(APP_BASE + i))                 ' 実インデックス 4 ～ 579
        Next
    ...
    Private Function BuildTankingResponse(reqHeader As Byte(),
        ...
        Buffer.BlockCopy(BitConverter.GetBytes(appCount), 0, header, 24, 4) ' AppCount
        Buffer.BlockCopy(BitConverter.GetBytes(appSize), 0, header, 28, 4)  ' AppSize
        ...
        ' 4. サム値計算 (4バイト)
        Dim sum As UInteger = 0UI
        For i As Integer = 4 To 15    ' アプリデータ部(ステータス～予備)
            sum += CUInt(data(i))
        Next
